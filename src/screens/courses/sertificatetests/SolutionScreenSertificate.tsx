...
const HeaderRight = React.memo(({ percent }: { percent: string }) => (
...
export default function SolutionScreenSertificate({
  ...
  const { themeId, userId, testId, percent, mavzu } = route.params;
  ...
  const currentAnswer = useMemo(() => {
    ...
    return answersById[currentQuestion.id];
  }, [answersById, currentQuestion]);
          ...
          <View style={styles.solutionViewContainer}>
            ...
            <View style={styles.solutionViewImageContainer}>
              {currentAnswer && currentAnswer.photos.length > 0 ? (
                <ImageViewer
                  key={`solution-cert-${currentAnswer.questionId}`}
...
const TestModal = React.memo(
  ({
    ...
    answersById: Record<number, LocalQuizAnswer>;
      ...
      ({ item }: { item: any }) => {
        ...
        return (
          <TestGridItem
            ...
            isCorrect={isCorrect}
            onSelect={() => answer && onTestSelect(answer)}
            styles={styles}
          />
        );
      },
      [answersById, currentQuestion, onTestSelect, styles],
    );
...
const ImageViewer = React.memo(
  ({
    ...
    const viewerId = useMemo(
      () =>
        `solution-cert-inline-${viewerKey}-${imageSignature}-${images.length}`,
