...
// MathLive engine + CSS + KaTeX fonts, bundled fully OFFLINE.
// (Generated by scripts/build-mathlive-offline.mjs — no runtime CDN/network.)
import { MATHLIVE_CSS, MATHLIVE_JS } from "@/src/assets/mathlive/mathliveAssets";
...
const MathLiveModalKeyboard = forwardRef<MathLiveModalRef, MathLiveModalProps>(
  ({ value, onChange, onClose }, ref) => {
    ...
    // HTML is built once. The MathLive engine, CSS and fonts are all inlined from
    // the offline bundle, so there is NO network request when the keyboard opens —
    // it loads instantly and works with no internet (no more stall/freeze).
    const mathliveHTML = useMemo(() => {
      // Inject the (self-contained) CSS via textContent so a stray "</style>"
      // inside the engine can never break the document.
      const cssLiteral = JSON.stringify(MATHLIVE_CSS);
      ...
      <head>
        ...
        <script>
          (function () {
            ...
            // Fonts are provided by the embedded CSS above; sounds are unused.
            // Setting these to null stops MathLive from fetching them over the network.
            if (window.MathfieldElement) {
              try { window.MathfieldElement.fontsDirectory = null; } catch (e) {}
        ...
        <script>
          (function() {
            try {
              ...
              // MathLive sozlamalari (offline — fontsDirectory tarmoqqa chiqmaydi)
              mf.setOptions({
          ...
          <SafeAreaView style={styles.container}>
            ...
            <View style={styles.webviewContainer}>
              <WebView
                ...
                scrollEnabled={false}
                originWhitelist={["*"]}
                // Everything is inlined → no network. Hardware layer keeps the
                // math rendering smooth on Android.
